...
public class NicknameCommand {
                                ...
                                .then(Commands.literal("solid")
                                        .then(Commands.argument("color", StringArgumentType.string())
                                                .then(Commands.argument("name", StringArgumentType.greedyString())
                                                        .executes(ctx -> setSolid(ctx, EntityArgument.getPlayers(ctx, "targets"))))))
                                .then(Commands.literal("gradient")
                                        .then(Commands.argument("startColor", StringArgumentType.string())
                                                .then(Commands.argument("endColor", StringArgumentType.string())
                                                        .then(Commands.argument("name", StringArgumentType.greedyString())
                                                                .executes(ctx -> setGradient(ctx, EntityArgument.getPlayers(ctx, "targets"))))))))
                        .then(Commands.literal("solid")
                                .then(Commands.argument("color", StringArgumentType.string())
                                        .then(Commands.argument("name", StringArgumentType.greedyString())
                                                .executes(ctx -> setSolid(ctx, null)))))
                        .then(Commands.literal("gradient")
                                .then(Commands.argument("startColor", StringArgumentType.string())
                                        .then(Commands.argument("endColor", StringArgumentType.string())
                                                .then(Commands.argument("name", StringArgumentType.greedyString())
                                                        .executes(ctx -> setGradient(ctx, null)))))))
            ...
            if (targets != null && !targets.isEmpty()) {
                for (ServerPlayer target : targets) {
                    ...
                    ctx.getSource().sendSuccess(() -> Component.literal("Set nickname for ")
                            ...
                            .append(NicknameManager.buildDisplayName(target)), true);
                }
                return targets.size();
            } else {
    ...
    private static void validateColor(String colorStr) {
        ...
        if (hex.length() != 6) throw new IllegalArgumentException("Color must be 6 hex characters");
