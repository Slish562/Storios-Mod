...
public class ChatColorManager {
    ...
    private static final String NBT_KEY = "storios_mod:chat_color";
    ...
    public record ChatColorData(String colorType, String color, String startColor, String endColor) {

        public static ChatColorData solid(String color) {
            return new ChatColorData("solid", color, null, null);
        }

        public static ChatColorData gradient(String startColor, String endColor) {
            return new ChatColorData("gradient", null, startColor, endColor);
        }
    ...
    public static void setChatColor(ServerPlayer player, ChatColorData data) {
    ...
    public static void removeChatColor(ServerPlayer player) {
        getPersistedData(player).remove(NBT_KEY);
    }

    public static ChatColorData getChatColor(ServerPlayer player) {
    ...
    public static boolean hasChatColor(ServerPlayer player) {
        return getPersistedData(player).contains(NBT_KEY);
    }

    public static Component colorize(String text, ChatColorData data) {
        if (data.isGradient()) {
            return createGradient(text, parseColor(data.startColor()), parseColor(data.endColor()));
        } else if (data.isSolid()) {
            return createSolid(text, parseColor(data.color()));
        }
